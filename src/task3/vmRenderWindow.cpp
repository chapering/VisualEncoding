	...
	m_strfnFiberIdx("")
{
	...
	m_optionPanel->addButton( "CST" );
	m_optionPanel->addButton( "CG" );
	m_optionPanel->addButton( "CC" );
	m_optionPanel->addButton( "IFO" );
	m_optionPanel->addButton( "ILF" );
}
...
int CLegiMainWindow::mainstay()
{
	...
	setWindowTitle( "LegiDTI Perceptual Study - Task 3" );
...
void CLegiMainWindow::__uniform_halos()
{
	...
	/*
	vtkLegiPolyDataMapper* mapStreamTube = m_bFirstHalo? vtkLegiPolyDataMapper::New() :
			vtkLegiPolyDataMapper::SafeDownCast( m_haloactor->GetMapper() );
	*/
