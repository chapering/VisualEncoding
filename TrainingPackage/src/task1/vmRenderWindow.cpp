	...
	m_nFlip(0)
{
	...
	//m_render->SetBackground(51/255.0, 102/255.0, 153/255.0  );
	m_render->SetBackground( 102/255.0, 102/255.0, 102/255.0  );
	//m_render->SetBackground2( 220/255.0, 220/255.0, 220/255.0  );
	...
	*/
	...
	m_anstext = vtkSmartPointer<CVTKTextbox>::New();
	

	m_optionPanel = new COptionPanel("", centralwidget);
	m_optionPanel->setOutputStream( &m_cout );
	m_optionPanel->addButton( "Similar" );
	m_optionPanel->addButton( "1 is Higher" );
	m_optionPanel->addButton( "2 is Higher" );
	m_optionPanel->prompt() = true;
	m_optionPanel->hintBtnText() = true;
...
int CLegiMainWindow::mainstay()
{
	...
	setWindowTitle( "LegiDTI Perceptual Study - Task 1" );
	...
	__add_axes();
	//__add_colorsphere(CLSCHM_ALL_SYMMETRIC);
	__addScaleLegend();
	...
	m_anstext->setText("prompt: ");
	m_anstext->setUseType( UT_ANSTEXT );
	m_anstext->VisibilityOff();
	m_render->AddActor2D( m_anstext );
	...
	//m_render->AutomaticLightCreationOn();
	//m_render->UpdateLightsGeometryToFollowCamera();
...
void CLegiMainWindow::__renderLines(int type)
{
	...
	/*
	vtkSmartPointer<vtkDashedStreamLine> dashlines = vtkSmartPointer<vtkDashedStreamLine>::New();
	dashlines->SetInput ( m_pstlineModel->GetOutput() );
	dashlines->SetSource( m_pstlineModel->GetOutput() );
	...
	filter->SetInput( m_pstlineModel->GetOutput() );
	glypher->SetInputConnection ( filter->GetOutputPort() );
	glypher->SetSourceConnection( arrows->GetOutputPort() );
	glypher->SetOrientationModeToRotation();
	glypher->SetScaleFactor(0.005);
	*/
	if ( SHAPE_SUPERELLIPSOID == m_nShapeType ) {
		vtkTgDataReader* pstlineModel = new vtkTgDataReader;
		if ( !pstlineModel->Load2( m_fnGeometry.c_str(), true, 6, 1.0) ) {
			return;
		}

		vtkDataArray* faArray = pstlineModel->GetOutput2()->GetPointData()->GetArray("FA COLORS");
		if ( m_bDepthColorLAB ) {
			__getLABColorArrayFromFAcolorArray(faArray, faArray);
		}

		mapStreamLines->SetInput( pstlineModel->GetOutput2() );
		mapStreamLinesHalo->SetInput( pstlineModel->GetOutput2() );
	...
	//mapStreamLines->SetInputConnection( dashlines->GetOutputPort() );
	//mapStreamLines->SetInput( glypher->GetOutput() );
	...
	//streamLineActor ->SetMapper(glypher);

	streamLineActor->GetProperty()->BackfaceCullingOn();
	streamLineActor->GetProperty()->SetLineWidth(2.0);
	//streamLineActor->GetProperty()->SetColor(102/255.0, 102/255.0, 102/255.0);
	streamLineActor->GetProperty()->SetColor(220/255.0, 220/255.0, 220/255.0);
	/*
	streamLineActor->GetProperty()->SetLineStippleRepeatFactor(10.0);
	streamLineActor->GetProperty()->SetLineStipplePattern(0xfff0);
	*/
...
void CLegiMainWindow::__uniformTubeRendering()
{
	...
	if ( m_bDepthSize ) {
		...
		*/
		...
		/*
		int __i = 2;
		linesort->GetOutput()->GetPointData()->AddArray( m_pstlineModel->GetOutput()->GetPointData()->GetArray("FA COLORS", __i) );
		*/
	...
	//tubeProperty->SetColor(102/255.0, 102/255.0, 102/255.0);
	tubeProperty->SetColor(220/255.0, 220/255.0, 220/255.0);

	mapStreamTube->ImmediateModeRenderingOn();

	if ( m_bDepthSize ) {
		mapStreamTube->StaticOff();
	}
	else {
		mapStreamTube->StaticOn();
	}
	/*
	vtkPolyDataMapper* mapStreamTube = vtkPolyDataMapper::SafeDownCast( m_actor->GetMapper() );
	vtkProperty* tubeProperty = m_actor->GetProperty();
...
void CLegiMainWindow::__add_axes()
{
	...
	*/
	...
	if ( m_nFlip ) {
		axes2->SetZAxisLabelText( "I" );
		axes2->SetXAxisLabelText( "L" );
	}
	else {
	...
	//m_orientationWidget->SetViewport( -0.2, -0.2, 0.4, 0.4 );
...
void CLegiMainWindow::__add_colorsphere(int schemeIdx)
{
	// add a colormap sphere
	...
	vtkPolyData* pdata = sphere->GetOutput();
	vtkPoints* points = pdata->GetPoints();
	vtkIdType ptnum = pdata->GetNumberOfPoints();
	vtkSmartPointer<vtkUnsignedCharArray> lut = vtkSmartPointer<vtkUnsignedCharArray>::New();
	lut->SetNumberOfComponents(3);
	lut->SetName("mapcolor");
	//cout << ptnum << " points in the sphere.\n";
	//cout << pdata->GetNumberOfCells() << " cells in the sphere.\n";

	double r,g,b;
	double pt[3];
	CColorMapper<double> cm(schemeIdx);
	for ( vtkIdType i = 0; i < ptnum; i++ ) {
		points->GetPoint(i, pt);
		cm.getColor(pt[0], pt[1], pt[2], r, g, b);
		lut->InsertNextTuple3(r*255,g*255,b*255);
		//lut->InsertNextTuple3(fabs(pt[2]), fabs(pt[1]), fabs(pt[0]));
	}
	sphere->GetOutput()->GetPointData()->AddArray(lut);
	sphere->GetOutput()->GetPointData()->SetActiveScalars("mapcolor");

	vtkSmartPointer<vtkPolyDataMapper> spmapper = vtkSmartPointer<vtkPolyDataMapper>::New();
	vtkSmartPointer<vtkActor> sphereactor = vtkSmartPointer<vtkActor>::New();
	spmapper->SetInputConnection( sphere->GetOutputPort() );
	spmapper->SetScalarModeToUsePointFieldData();
	spmapper->SelectColorArray("mapcolor");
	spmapper->ScalarVisibilityOn();
	sphereactor->SetMapper(spmapper);
	...
	*/
	...
	axes2->SetTotalLength( 4.5, 4.5, 4.5 );
	axes2->SetCylinderRadius( 0.500 * axes2->GetCylinderRadius() );
	axes2->SetConeRadius    ( 1.025 * axes2->GetConeRadius() );
	axes2->SetSphereRadius  ( 1.500 * axes2->GetSphereRadius() );
...
{
	int nBoxes = (int)m_strfnBoxes.size();
	int nLineInbox = 0;

	string discard;
	double* faValues = new double[nBoxes];
	double bounds[6];
	//cout << "FAInfo: ";
	for (int i = 0; i < nBoxes; i++) {
		...
		*/
		...
		//tfilter->SetInputConnection( box->GetOutputPort() );
		...
		//vtkSmartPointer<vtkProperty> boxproperty = vtkSmartPointer<vtkProperty>::New();
		...
		boxproperty->SetColor(1,1,0);
		boxproperty->SetOpacity(0.5);
		...
		boxactor->SetProperty( boxproperty );

		m_render->AddActor( boxactor );
		...
		boxproperty2->SetColor(1.0,153.0/255.0,0.0);
		...
		/*
		vtkCubeSource* box2 = vtkCubeSource::New();
		box2->SetBounds( bounds );

		vtkSmartPointer<vtkTransformPolyDataFilter> tfilter2 = vtkSmartPointer<vtkTransformPolyDataFilter>::New();
		vtkSmartPointer<vtkTransform> transform2 = vtkSmartPointer<vtkTransform>::New();

		transform2->Translate( (extent[0] + extent[1])/2.0, (extent[2] + extent[3])/2.0, (extent[4]+extent[5])/2.0 );

		tfilter2->SetInputConnection( box2->GetOutputPort() );
		...
		*/
		...
		//_calBlockAvgFA( extent, nLineInbox );

		faValues[i] = _calBlockAvgFA(bounds, nLineInbox);
		//cout << faValues[i] << " " << nLineInbox << " ";
	}

	m_nKey = (faValues[0] > faValues[1])?2:3;

	double faDiff = fabs(faValues[0] - faValues[1]);
	if (faDiff < 1e-1 ) {
		m_nKey = 1;
	}
...
void CLegiMainWindow::__drawCanvas(bool bGrid)
{
	...
	//canvasRenderer->SetBackground(220/255.0, 220/255.0, 220/255.0  );
	//canvasRenderer->SetBackground( 51/255.0, 102/255.0, 153/255.0  );
	...
	*/
	...
	//canvasProp->SetColor( 60.0/255, 60.0/255, 60.0/255 );
	...
	while ( alllines->GetNextCell(szPts, line) ) {
		...
		for ( vtkIdType ptIdx = 0; ptIdx < szPts + 6; ptIdx += 6 ) {
			...
			*/
			...
			/* this rotation has been sticking me for about 2 hours.. and torturing my brain */
			/* the elixir: default orientation of a superEllipse is NOT 1,0,0 BUT 0,0,1, a long "trial and error"
			 * reveals this, however
			 */
			...
			if ( Direction[0] < 0.0 )
			{
				...
				/*
				transform->RotateWXYZ(ang, (Direction[0]-vMag)/2.0,
			...
			if ( szPts - 1 - ptIdx > 3 || szPts - 1 == ptIdx ) {
