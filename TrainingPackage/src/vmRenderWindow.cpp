	...
	*/
	...
	switch (m_nShapeType) {
		case SHAPE_CONE:
			...
			if ( m_bDepthColor ) {
				__insertFiberMarkersInFAColors(faArray);
			}
...
void CLegiMainWindow::__drawCones()
{
	...
	while ( alllines->GetNextCell(szPts, line) ) {
		...
		for ( vtkIdType ptIdx = 0; ptIdx < szPts + 6; ptIdx += 6 ) {
			...
			conesrc->SetRadius((1-faColor[1])*1.5);
			conesrc->SetHeight((1-faColor[1])*1.5);
...
void CLegiMainWindow::__insertFiberMarkersInFAColors(vtkDataArray* out)
{
	vtkPolyData* tubes = m_pstlineModel->GetOutput();
	vtkCellArray* alllines = tubes->GetLines();
	...
	while ( alllines->GetNextCell(szPts, line) ) {
		...
		if (bKeyFiber) {
			for (vtkIdType j = 0; j < szPts; j++) {
				out->SetTuple3( tupleIdx + j, 255, 255, 0 );
			}
		}
