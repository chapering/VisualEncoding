	...
	m_strfnFiberIdx("")
{
	...
	*/
	...
	m_optionPanel->addButton( "Box 1" );
	m_optionPanel->addButton( "Box 2" );
	m_optionPanel->addButton( "Box 3" );
...
int CLegiMainWindow::mainstay()
{
	...
	setWindowTitle( "LegiDTI Perceptual Study - Task 2" );
	...
	switch ( m_nOrientationType ) {
		case ORT_COLOR:
			...
			break;
		case ORT_SAT:
			__add_colorsphere(CLSCHM_SATURATION);
			break;
		case ORT_NONE:
		default:
			__add_axes();
...
void CLegiMainWindow::__uniformTubeRendering()
{
	...
	if ( m_bDepthSize ) {
		...
		*/
		...
		switch ( m_nOrientationType ) {
			case ORT_COLOR:
				m_pstlineModel->GetOutput()->GetPointData()->SetActiveScalars("orientation colors");
				break;
			case ORT_SAT:
				m_pstlineModel->GetOutput()->GetPointData()->SetActiveScalars("saturation colors");
				break;
			case ORT_NONE:
			default:
				m_pstlineModel->GetOutput()->GetPointData()->SetActiveScalars("FA COLORS");
				break;
		}
	}
	...
	*/
	if ( m_bDepthColor ) {
		...
		switch ( m_nOrientationType ) {
			case ORT_COLOR:
				mapStreamTube->SetScalarModeToUsePointFieldData();
				mapStreamTube->SelectColorArray("orientation colors");
				mapStreamTube->ScalarVisibilityOn();
				break;
			case ORT_SAT:
				mapStreamTube->SetScalarModeToUsePointFieldData();
				mapStreamTube->SelectColorArray("saturation colors");
				mapStreamTube->ScalarVisibilityOn();
				break;
			case ORT_NONE:
			default:
			...
			case QXmlStreamReader::StartElement:
				{
					...
					else if ( sreader.name().compare("orientation") == 0 ) {
						curTag = VN_ORIENTATION;
					}
					...
					else if ( sreader.name().compare("otype") == 0 ) {
						curSubTag = VSN_OTYPE;
					}
					else {
			...
			case QXmlStreamReader::Characters:
				{
					...
					switch ( curTag ) {
						...
						case VN_ORIENTATION:
							switch (curSubTag) {
								case VSN_OTYPE:
									if (strtext.compare("NONE") == 0) {
										m_nOrientationType = ORT_NONE;
									}
									else if (strtext.compare("SHAPE") == 0) {
										m_nOrientationType = ORT_SHAPE;
									}
									else if (strtext.compare("COLOR") == 0) {
										m_nOrientationType = ORT_COLOR;
									}
									else if (strtext.compare("SAT") == 0) {
										m_nOrientationType = ORT_SAT;
									}
									break;
								default:
									break;
...
int CLegiMainWindow::_loadFiberIdx()
{
...
void CLegiMainWindow::_drawMarkers()
{
	//GLfloat x, y, z;
	vtkPolyData* tubes = m_pstlineModel->GetOutput();
	vtkCellArray* alllines = tubes->GetLines();
	//cout << "totally " << alllines->GetNumberOfCells() << " lines.\n";
	...
	std::set<vtkIdType>::iterator it = m_expectedFiberIndices.begin();
	vtkIdType sIdx = 0, eIdx, lineIdx = -1, curIdx;
	double *center = m_boxes[m_nKey - 1]->GetCenter();
	point_t keyBoxCenter(center[0], center[1], center[2]);
	...
	keyBoxCenter = keyBoxCenter + offset;

	alllines->InitTraversal();
	while ( alllines->GetNextCell(szPts, line) && it != m_expectedFiberIndices.end()  ) {
		curIdx = *it; 

		lineIdx ++;
		if ( curIdx != lineIdx ) continue;

		sIdx = 0;
		eIdx = szPts - 1;
		//cout << "in line " << curIdx << ": " << sIdx << "," << szPts << "\n";
		//cout << "in line " << curIdx << ": " << line[sIdx] << "," << line[eIdx] << "\n";

		tubes->GetPoint( line[sIdx], point );
		fiberstart.update ( point[0], point[1], point[2] );

		tubes->GetPoint( line[eIdx], point );
		fiberend.update ( point[0], point[1], point[2] );
		...
		sph->SetCenter( markPt.x, markPt.y, markPt.z );
		sph->SetRadius(.5);
		sph->SetThetaResolution(50);
		sph->SetPhiResolution(50);

		vtkSmartPointer<vtkPolyDataMapper> smap = vtkSmartPointer<vtkPolyDataMapper>::New();
		smap->SetInputConnection( sph->GetOutputPort() );

		vtkSmartPointer<vtkActor> sactor = vtkSmartPointer<vtkActor>::New();
		sactor->SetMapper(smap);
		//sactor->GetProperty()->SetColor(1,1,0);
		sactor->GetProperty()->SetColor(1.0, 153/255.0, 0.0);
