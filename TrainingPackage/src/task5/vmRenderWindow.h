...
public slots:
	...
	vtkSmartPointer<CVTKTextbox>& getAnsText() { return m_anstext; }
private:
	imgVolRender* m_pImgRender;
	vtkTgDataReader* m_pstlineModel;
	vtkTgDataReader* m_pstlineModel2;
	...
	vtkSmartPointer<CVTKTextbox> m_anstext;
	...
	std::set<vtkIdType> m_expectedFiberIndices;
private:
	/**
...
private:
	...
	// mark the highlighted fiber tubes
	vtkLookupTable* __markFibers(vtkPolyData* tubes);

	void __drawSuperEllipsoids();
	void __drawCones();

	void __getLABColorArrayFromFAcolorArray(vtkDataArray* in, vtkDataArray* out);
	void __addColorBar();
	void __insertFiberMarkersInFAColors(vtkPolyData* tubes, vtkDataArray* out);

	point_t m_scaleMinMax;
	void __addScaleLegend();
};
